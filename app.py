...
# --- FUNÇÃO PARA CONVERSÃO DE IMAGENS EM BASE64 ---
...
# --- FUNÇÕES AUXILIARES PARA MEMÓRIA DE CÁLCULO ---
def salvar_historico_completo(prefixo, val_luz, total_kwh, leitura_ant, leitura_at, val_agua):
    caminho_txt = os.path.join(PASTA_DESTINO, f"{prefixo} - valores.txt")
    with open(caminho_txt, "w") as f:
        f.write(f"{val_luz}\n")
        f.write(f"{total_kwh}\n")
        f.write(f"{leitura_ant}\n")
        f.write(f"{leitura_at}\n")
        f.write(f"{val_agua}\n")

def carregar_historico_completo(prefixo):
    caminho_txt = os.path.join(PASTA_DESTINO, f"{prefixo} - valores.txt")
...
def buscar_leitura_atual_anterior(mes_selecionado):
    try:
        ...
        if idx > 0:
            ...
            dados = carregar_historico_completo(prefixo_ant)
            return dados[3] 
    except:
        pass
    return 0.0
...
# ==========================================
    ...
    if st.session_state.perfil == "admin":
        ...
        with aba_subir:
            ...
            luz_fatura, kwh_fatura, medidor_ant, medidor_at, agua_fatura = carregar_historico_completo(prefixo_data)
            ...
            if st.button("Salvar e Publicar no Sistema"):
                salvar_historico_completo(prefixo_data, val_luz, total_kwh, leitura_ant, leitura_at, val_agua)
                
                if arquivo_luz is not None:
                    extensao = os.path.splitext(arquivo_luz.name)[1].lower()
                    nome_final_luz = f"{prefixo_data} - luz{extensao}"
                    with open(os.path.join(PASTA_DESTINO, nome_final_luz), "wb") as f:
                        f.write(arquivo_luz.getbuffer())
                
                if arquivo_agua is not None:
                    extensao = os.path.splitext(arquivo_agua.name)[1].lower()
                    nome_final_agua = f"{prefixo_data} - água{extensao}"
                    with open(os.path.join(PASTA_DESTINO, nome_final_agua), "wb") as f:
                        f.write(arquivo_agua.getbuffer())
                
                st.success(f"Dados e fórmulas de {mes_ano} publicados com sucesso!")
    ...
    # ------------------------------------------
        ...
        # Mensagem de boas-vindas personalizada de acordo com quem logou
        ...
        val_luz, total_kwh, leitura_ant, leitura_at, val_agua = carregar_historico_completo(codigo_mes)
        
        # CÁLCULO DA ENERGIA COM BASE NO USUÁRIO LOGADO
        if total_kwh > 0 and leitura_at >= leitura_ant:
            ...
            if st.session_state.perfil == "marido":
                # Se for o Marido logado, ele paga o consumo direto do medidor interno (Dry/Rafa)
                parte_luz_exibida = custo_dry_rafa
                titulo_detalhe = "📝 Detalhamento dos Gastos (Dry / Rafa)"
            else:
                # Se for o Vicente logado, ele paga o restante (Gaby/Mandy)
        ...
        # 2. GIF do Travolta (Coadjuvante menor e centralizado)
        ...
        # 3. Box da Nazaré (Puxando a matemática inteligente separada por perfil)
        st.markdown(f"""
        ...
        arquivo_luz_encontrado = None
        for ext in ['.pdf', '.jpg', '.jpeg', '.png']:
            nome_tentativa = f"{codigo_mes} - luz{ext}"
            caminho_tentativa = os.path.join(PASTA_DESTINO, nome_tentativa)
            if os.path.exists(caminho_tentativa):
                arquivo_luz_encontrado = caminho_tentativa
                break
        
        with col_down_1:
            if arquivo_luz_encontrado:
                with open(arquivo_luz_encontrado, "rb") as file:
                    st.download_button(
                        label="📥 Fatura Luz",
                        data=file,
                        file_name=os.path.basename(arquivo_luz_encontrado),
                        mime="application/octet-stream"
                    )
            else:
                st.info("ℹ️ Luz ausente.")
                
        arquivo_agua_encontrado = None
        for ext in ['.pdf', '.jpg', '.jpeg', '.png']:
            nome_tentativa = f"{codigo_mes} - água{ext}"
            caminho_tentativa = os.path.join(PASTA_DESTINO, nome_tentativa)
            if os.path.exists(caminho_tentativa):
                arquivo_agua_encontrado = caminho_tentativa
                break
                
        with col_down_2:
            if arquivo_agua_encontrado:
                with open(arquivo_agua_encontrado, "rb") as file:
                    st.download_button(
                        label="📥 Fatura Água",
                        data=file,
                        file_name=os.path.basename(arquivo_agua_encontrado),
                        mime="application/octet-stream"
                    )
            else:
